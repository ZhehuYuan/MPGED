...
public:
	...
	countRead = new int[nThread]();
	countExplore = new int[nThread]();
    }
...
public:
	...
	countRead = new int[_t]();
	countExplore = new int[_t]();
    }
...
int main(int argc, char* argv[]) {
	...
	if(!multi->finish){printf("unfinished\n");return 1;}
	    ...
	    int countExplore = 0;
	    for(int i = 0; i<nThread; i++){
		countRead += multi->countRead[i];
		countExplore += multi->countExplore[i];
	    }
	    printf("%f\t%f\n", double(countRead/nThread), double(countExplore/nThread));
        }
        else {
            endTime = std::chrono::steady_clock::now();   
        }
