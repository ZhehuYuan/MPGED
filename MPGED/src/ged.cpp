...
class GEDCPQ {
public:
    tbb::concurrent_priority_queue<node> cpqHeap;
    mutex lock;
    int nHeap;
    int totalThread;
    mutex lockUb;
    ...
    GEDCPQ(int _nHeap, int nThread, int _l1, int _l2, int* _V1, int* _V2, int* _E1, int* _E2, int _maxEdge, int _maxVertex) {
	finish = true;
        memUsed = new bool[preAllocSize]();
        for (int i = 0; i < preAllocSize; i++) { memUsed[i] = 0; }
        memG1 = new int[preAllocSize*_l1]();
    ...
    node findBest(int waitState, int id) {
	    node ret;
	    if(!cpqHeap.try_pop(ret)){
	        ret = emptyNode;
		if(waitState == 0){
		    waiting.fetch_add(1);
		}
	    }else if(ret.data.lb >= upperBound){
	        ret = emptyNode;
		if(waitState == 0){
		    waiting.fetch_add(1);
		}
...
void middle5(int id, GEDCPQ* multi) {
    set_affinity(id+1);
    multi->aStar(id);
}
...
int main(int argc, char* argv[]) {
    ...
    if(argc != 5){
    	printf("error, input should be <data file> <# vertices> <# thread> <method Id>\n");
	return 0;
    }else{
        data->open(argv[1]);
        string x(argv[2]);
    ...
    if (methodId == 0) {
        GED* multi = new GED(l1, l2, V1, V2, E1, E2, maxLabelEdge, maxLabelVertex);
        ...
        multi->memTop++;
        multi->memUsed[0] = 1;
        tmp.stabledMatch = multi->memG1;
        for (int j = 0; j < l1; j++) {
            tmp.stabledMatch[j] = -1;
        }
        tmp.stabledMatch2 = multi->memG2;
        for (int j = 0; j < l2; j++) {
        ...
        for (int i = 0; i < l2; i++) {
            ...
            tmp.unusedData.push(data);
        }

        tmp.data = tmp.unusedData.top();
        tmp.unusedData.pop();
        tmp.stabledMatch[0] = tmp.data.newMatch;
        tmp.stabledMatch2[tmp.data.newMatch] = 0;
        multi->heap->push(tmp);
	
	thread* x = new thread(middle4, multi);
	...
	if(!multi->finish){printf("unfinished\n");return 1;}
    }
    else if (methodId == 1) {
	...
	thread* x;
        ...
        GEDCPQ* multi = new GEDCPQ(int(nThread), nThread, l1, l2, V1, V2, E1, E2, maxLabelEdge, maxLabelVertex);
        ...
        for (int i = 0; i < l2; i++) {
            ...
            tmp.stabledMatch[0] = tmp.data.newMatch;
            tmp.stabledMatch2[tmp.data.newMatch] = 0;
	    multi->cpqHeap.push(tmp);
        }

        thread* x;
        for (int i = 0; i < nThread; i++) {
            x = new thread(middle5, i, multi);
	    x->detach();
        }
