...
public:
    ...
    MPGED(int _nHeap, int nThread, int _l1, int _l2, int* _V1, int* _V2, int* _E1, int* _E2, int _maxEdge, int _maxVertex, int _N) {
...
int main(int argc, char* argv[]) {
    ...
    if(argc != 5){
    	printf("your input should be: <data file name> <# vertices> <# thread> <N>\n");
	return 0;
    }else{
        data->open(argv[1]);
        string x(argv[2]);
	...
	if(nThread==64 || nThread==128 || nThread==192 || nThread==256)nThread-=1;
        MPGED* multi = new MPGED(nThread, nThread, l1, l2, V1, V2, E1, E2, maxLabelEdge, maxLabelVertex, N);
	...
	int countHS = 0;
	for (int i = 0; i < nThread; i++){
	    countCAS += multi->countCAS[i];
	    countHS += multi->countHS[i];
	}
	printf("%d\t%d\n", countCAS, countHS);
}
